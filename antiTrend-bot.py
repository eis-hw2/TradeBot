...
class ACCOUNT:
    ...
    def getQuotation(self):
        while 1:
            if 0 not in self.c_Y:
                self.sell_smallInterval -= self.c_Y*0.01
                self.buy_largeInterval -= self.c_Y*0.01
                self.sell_largeInterval += self.c_Y*0.01
                self.buy_smallInterval += self.c_Y*0.01
            time.sleep(5)
            bId = self.brokers[0]
